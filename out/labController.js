    ...
    async startLabFromUri(server, coursePath) {
        ...
        vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Loading lab…' }, async () => {
            ...
            if (!ghSession) {
                ...
                const signIn = await vscode.window.showWarningMessage('The interactive lab companion requires a GitHub Copilot license. Please sign in to GitHub to continue.', 'Sign In');
                if (signIn === 'Sign In') {
                ...
                if (!ghSession) {
                    ...
                    vscode.window.showErrorMessage('A GitHub Copilot license is required for the lab companion. You can still browse course slides from the catalog.');
                    return;
                }
            }
